...
func (q *QueryFeedVideoListFlow) checkData() {
	// 上层通过把userId置零，表示userId不存在或不需要
	if q.userId > 0 {
		// 说明这里userId是有效的，可以定制性的做一些登陆用户的专属视频推荐
	}
...
func (q *QueryFeedVideoListFlow) prepareData() error {
	err := model.NewVideoDAO().QueryVideoListByLimitAndTime(MaxVideoNum, q.latestTime, &q.videos)
	if err != nil {
		return err
	}
	// 如果用户为登陆状态，则更新该视频是否被该用户点赞的状态
